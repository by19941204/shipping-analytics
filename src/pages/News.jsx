    ...
    <div className="max-w-6xl mx-auto">
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-2xl font-bold text-[var(--text-primary)]">{t('news.title')}</h1>
          </div>
          <div className="flex items-center gap-3">
            {/* Live indicator */}
            ...
            <button
              onClick={refresh}
              disabled={loading}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium bg-[var(--bg-card)]/50 text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-card-hover)]/50 transition-all duration-200 cursor-pointer disabled:opacity-50"
            >
        ...
        <div className="flex items-center justify-between">
          <p className="text-[var(--text-secondary)] text-sm">{t('news.subtitle')}</p>
          {lastFetched && (
      ...
      {loading && news.length === 0 && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          {[1,2,3,4,5,6].map(i => (
            <div key={i} className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl p-5 animate-pulse">
              <div className="h-4 bg-[var(--bg-card)] rounded w-20 mb-3" />
              <div className="h-5 bg-[var(--bg-card)] rounded w-3/4 mb-2" />
              <div className="h-4 bg-[var(--bg-card)] rounded w-full mb-1" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        {filtered.map((article) => {
          ...
          return (
            <article
              key={article.id}
              className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl p-5 hover:border-[var(--border-color)] hover:bg-[var(--bg-card-hover)] transition-all duration-200 hover:-translate-y-0.5 hover:shadow-lg hover:shadow-black/20"
            >
              ...
              {article.link ? (
                <a href={article.link} target="_blank" rel="noopener noreferrer" className="block group">
                  <h2 className="text-[var(--text-primary)] font-semibold leading-snug mb-2 group-hover:text-blue-400 transition-colors">
                    {title}
                  </h2>
                </a>
              ) : (
                <h2 className="text-[var(--text-primary)] font-semibold leading-snug mb-2">
                  {title}
                </h2>
              )}
